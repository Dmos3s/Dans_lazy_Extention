...
- Work in **tiny batches** (5-8 emails max). After each batch: progress_update for the ones you handled, then progress_read to verify state (especially after any compaction).
...
- Gmail search bar: the input has placeholder="Search mail" (or aria-label). text click({text:"Search mail"}) can be ambiguous (matches the search button too). Prefer: get_accessibility_tree({filter:"interactive", maxDepth:5}) near top, find the input with placeholder containing "Search", use its ref_id with click_ax({ref_id}) or set_field directly. Or after ambiguity, use the candidate whose tag="input" or has placeholder.
