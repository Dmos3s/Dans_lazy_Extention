...
export default {
  ...
  'ob.tokens.context_hint': "为了让智能体可靠运行，请选择上下文窗口至少为 16k 个 token 的模型。启用紧凑模式后 8k 也可使用；4k 对于该工具集来说太小。Doll 会在接近窗口上限时自动压缩对话。",
  ...
  'ob.welcome.title': '欢迎使用 Doll',
  'ob.welcome.body': 'Doll 是一个位于侧边栏中的 AI 浏览器代理。它可以阅读页面、回答问题、填写表单、点击按钮，并在网页上自动执行多步骤任务。',
  'ob.welcome.note': '开始之前，先快速了解几件事。',
  'ob.act.title': '使用执行模式前请先确认',
  'ob.act.body': 'Doll 的执行模式可以代表你采取真实操作。继续之前，你需要负责检查提示、目标位置和执行结果。',
  'ob.act.warn1': 'Doll 可能会截取屏幕用于规划。除非你接受风险，否则请避免在敏感页面使用代理模式，例如健康、金融、约会或私人账户门户。',
  'ob.act.warn2': '网页内容可能包含恶意提示注入，试图操纵 AI 行为。',
  'ob.act.warn3': '请始终检查建议的操作，并停止任何看起来异常的运行。',
  'ob.tokens.title': '选择模型',
  'ob.tokens.body': 'Doll 需要语言模型才能工作。我们会先检查本地模型。如果没有可用模型，请打开设置并添加提供商：',
  'ob.tokens.scanning': '正在检查本地模型...',
  'ob.tokens.local_body': '此电脑上检测到可用的本地模型。选择一个以启用它，并立即开始使用 Doll。',
  ...
  'ob.tokens.option_lmstudio': 'LM Studio：免费在本地运行模型。启动 LM Studio，加载模型后 Doll 会自动连接。',
  ...
  'sp.inspection': 'Doll 已开始检查此页面',

  'sp.greeting.html': '你好！我是 <strong>Doll</strong>。我可以阅读此页面、回答问题、点击按钮、填写表单以及执行多步骤自动化任务。',
  ...
  'sp.mode.act': '执行',
  'sp.mode.act.title': '让 Doll 代你点击、输入和导航',
  'sp.mode.act.warning': '执行模式：Doll 将代你操作页面。请自行承担风险。',
  'sp.mode.act.confirm': '执行模式允许 Doll 代你点击、输入、滚动和导航。\n\n它在你已登录的浏览器会话中运行，因此在每个你登录的站点上拥有和你一样的权限。\n\n请注意观察其行为，并可随时通过 ◼ 按钮停止。\n\n是否继续？',
  ...
  'sp.subscribe.allowance_used': '今日免费的 Doll Cloud 额度已用完。',
  'sp.subscribe.btn': '订阅',
  ...
  'st.display.language.desc': 'Doll 插件的界面语言。',
  ...
  'st.providers.webbrain_note.body': '免费的每日 Doll Cloud 用量已包含在内。请求通过 api.webbrain.one 处理；默认情况下，我们仅记录用于配额和调试的元数据，不记录提示文本、页面内容、屏幕截图或模型响应。{privacyLink}。如需更多用量，请在 {subscribeLink} 订阅。在 {accountLink} 管理账单。',
  ...
  'st.profile.security_html': '<strong>安全提示 — 请阅读：</strong><br><ul style="margin:6px 0 0 18px;padding:0;line-height:1.55;"><li>你在这里输入的文本以<strong>明文</strong>保存在浏览器的本地存储中。它<strong>不会</strong>被发送到 Doll 项目 — 但<strong>会</strong>在每一轮作为系统提示的一部分发送给你所配置的 LLM 提供商。</li><li><strong>不要在这里放重要账户的密码</strong>（Google、Apple、iCloud、银行、工作 SSO、主邮箱）。这些账户应启用 2FA，也本就不该交给代理。</li><li>适合此处的是你用于低风险注册（时事通讯、免费试用、论坛账户）的<strong>一次性密码</strong>。</li><li>如果此浏览器配置文件被攻破，能访问磁盘的攻击者就能读到此文本。请尽量精简。</li></ul>',
  ...
  "sp.perm.question": "Doll 想要{verb} {host}。允许吗？",
  "sp.perm.reason": "权限检查 —— Doll 仅在你允许的站点上执行重要操作。",
  ...
  "st.perms.gate.desc": "推荐开启。开启后，Doll 首次在某站点点击、输入、导航、运行 JavaScript、上传、下载或录制时会请求权限（随后记住你的选择）。在你完全信任的站点上可关闭，以便快速、免提示地使用。",
  "st.perms.gate.warning": "⚠️ 权限提示已关闭。Doll 将在任何站点上点击、输入、导航、运行 JavaScript、上传、下载和录制而无需询问 —— 包括内容可能被攻击者控制的页面。页面内容在内部仍被视为不可信，但没有任何机制能阻止被注入的指令触发操作。只有当你完全信任使用本助手的每一个站点时，才将其保持关闭。",
  "st.perms.desc": "你已允许 Doll 在其上操作的站点。每一条对应授予某个特定站点的一项能力（点击、输入、导航、运行 JavaScript 等）。移除任何你不再需要的项 —— 下次 Doll 需要时会再次询问。",
  "st.perms.empty": "尚未授予任何站点权限。Doll 会在某站点上点击、输入、导航、上传、下载、录制或运行代码前先询问，并在你选择“始终允许”时把你的选择记在这里。",
  ...
  "st.display.strict_secret.desc": "拒绝在总结或助手文本中引用凭据（密码、API 密钥、令牌、OTP）—— 即使你明确要求也是如此。如果你经常分享轨迹文件或共享屏幕，这会很有用。默认关闭：Doll 运行在你自己的浏览器中，因此默认情况下代理会向你展示你索取的值，只是把 `done` 总结保持整洁。",
  ...
  'sp.schedule_form.prompt_placeholder': '运行时 Doll 应做什么？',
  ...
  'st.display.scheduled_tasks.desc': '允许 Doll 恢复延迟工作，并使用浏览器定时器运行一次性或周期性定时任务。默认开启。',
