...
- Bulk inbox processing (e.g. finding/sorting "BidNet Direct" emails): Gmail lists are virtualized. **Always start with get_state_digest** (cheap orientation). Then **immediately call extract_data({type:"tables"}) or extract_data({type:"links"})** for the visible rows — this is far better than raw AX tree for lists.
- **BULK ACTION ON MANY MATCHING EMAILS ("all", "every", "sort the inbox", hundreds of items) — PREFER GMAIL'S NATIVE SELECT-ALL-MATCHING, NOT A PER-ITEM LOOP:**
  1. Search first to scope the set precisely (from:, subject:, has:, etc. — see below).
  2. Click the checkbox at the top of the message list to select every row on the current page. Gmail then shows a banner: **"All N conversations on this page are selected. Select all conversations that match this search"** — that second part is a clickable link/button. **MANDATORY sequence for this click — do not skip step (a):**
     (a) Call get_accessibility_tree({filter:"interactive"}) (or extract_data({type:"links"})) and find the element whose text/name contains "Select all" or "match this search". This is a small text link, easy to miss visually — it is NOT optional to look it up, even if you already have a screenshot.
     (b) Click it via click_ax({ref_id}) using the ref_id from (a), or click({text:"Select all ... conversations that match this search"}) with textMatch:"contains".
     (c) If it does not appear in the tree (rare — selection state can lag a moment), re-call get_accessibility_tree once more before trying anything else. **Never click this banner by (x,y) coordinates** — it is a thin single-line link near the top of a virtualized list and coordinate guesses reliably miss it, which is exactly the pixel-click loop the runtime's loop-detector exists to catch.
  3. This selects EVERY matching conversation across all pages (Gmail's own cap is generous — thousands), not just the visible page. Now apply "Move to" / "Labels" / Archive / Delete **ONCE** — it applies to the whole selected set in one action.
  4. Verify by re-running the same search after the action — the result count should now be 0 (or reflect the new label).
  This turns a "hundreds of emails" task into ~5 tool calls total instead of dozens of per-item batches, avoids repeated compaction, and is what Gmail is actually built for. **Only fall back to the tiny-batch loop below when the task needs per-item judgment** (e.g. "reply differently to each one based on content", "flag only the suspicious ones") — a bulk select-all can't make per-row decisions.
- **FALLBACK — per-item judgment required, so bulk select-all doesn't apply**: enter chunked mode — keep the OVERALL GOAL + only the NEXT TINY CHUNK (5-8 items). After every small chunk use progress_update (unique IDs) + progress_read / digest. Never try to hold the full list in context. Use unique IDs in the ledger: prefer the email subject + date snippet, or the message thread URL/ID from extract_data (e.g. "#inbox/ABC123"). Never use a generic label as the id — every row would collide. After any context trim/compaction, always call progress_read before re-scanning so you don't re-process already handled items.
- **Gmail search bar — PREFER THE SELECTOR FORM, NOT click-then-type:** Gmail's search input has a stable, long-standing attribute: \`name="q"\`. Skip the click/focus dance entirely and call **type_text({selector: 'input[name="q"]', text: "from:bidnetdirect.com", clear: true})** directly — this queries the DOM for the element and sets its value via a native setter, which works regardless of whether the input is visually clickable, covered, or the viewport (e.g. a narrow side panel) makes it hard to hit with a simulated click. This is more reliable here than click_ax + type_ax/set_field, and MUCH more reliable than any coordinate click. Follow with press_keys({key:"Enter"}) or re-call with a submit-capable tool to run the search. Only fall back to get_accessibility_tree({filter:"interactive", maxDepth:5}) + click_ax({ref_id}) if the selector form fails (e.g. Gmail changed its markup) — and even then, never resort to (x,y) coordinates on this input; it sits in a dense top toolbar where pixel guesses reliably land on the wrong element.
- Only take action (click, move, label) after you have extracted and ledger-tracked the current visible set.
- **Moving / labeling selected emails**: After selecting (checkboxes, "Select all", or the select-all-matching banner above — you will see blue highlight and count in toolbar), the top action bar changes (shows "Move to", "Labels", trash icons). Click the "Move to" button (or "Labels") **using its ref_id from the current tree**. The menu opens as a popup/portal (often new items appear at the end of the accessibility tree or as a new listbox). **Immediately re-call get_accessibility_tree({filter:"visible"}) or get_state_digest + extract_data to see the new menu items and any filter input inside it.** Then use the fresh ref or type the folder/label name in the popup's input and press Enter / click the target. Do NOT guess coords from screenshot for the toolbar or menu — the refs change when selection state changes. If the menu doesn't appear after click, re-read the tree first before retrying.
- **Drag-and-drop fallback for labeling**: Gmail also supports dragging a selected email (or its row) directly onto a label name in the LEFT SIDEBAR to apply that label — a real Gmail feature, not a workaround. If the "Labels"/"Move to" popup keeps failing to resolve (ref not found after 1-2 re-reads, menu not rendering), use **drag_drop({fromRefId: <the email row's ref_id>, toRefId: <the label's ref_id in the left sidebar>, steps: 15})** instead of continuing to retry the popup. Read both refs fresh from get_accessibility_tree first — do not reuse refs from before the selection. **Do NOT click_ax/click the sidebar label instead** — clicking a label in the sidebar NAVIGATES to that label's view, it does NOT apply the label to your current selection; only a drag onto it does. This fallback only applies to labeling via the sidebar; it does not help with menu-only actions that have no sidebar equivalent (e.g. "Mark as read", "Snooze") — for those, keep retrying the popup/menu path.`,
  },
  {
    name: 'google-docs',
    category: 'general',
...
- "Edit message" works for a window after sending (~48h); "Delete for everyone" within a shorter window — both have explicit confirms.`,
  },
  {
    ...
    category: 'general',
    // Broad: any .gov subdomain, plus the well-known bid-aggregator portals
    // (BidNet Direct, SAM.gov, PlanetBids, DemandStar, Bonfire, Public
    // Purchase) that state/county/municipal buyers commonly white-label.
    // These sites vary wildly in markup (many are older ASP.NET WebForms
    // postback UIs, not SPAs) — so this note describes SHAPES and known
    // failure modes rather than selectors, per this file's own guidance.
    match: (url) => /^https?:\/\/[^/]*\.gov(\/|$|:)/i.test(url) ||
      /bidnetdirect\.com|sam\.gov|planetbids\.com|publicpurchase\.com|bonfirehub\.com|demandstar\.com/i.test(url),
    notes: `
- These are search/filter/result-list sites, not editors — the recurring task shape is: set search criteria (keywords, category/NAICS code, date range, region) → read a paginated results table → extract rows → repeat across pages or searches. Treat it exactly like the Gmail bulk-list pattern: **get_state_digest for orientation, then extract_data({type:"tables"}) for the result rows** — do not read the whole page via get_accessibility_tree for a long results list.
- Many of these (especially plain .gov county/state sites) are OLDER server-rendered forms (ASP.NET WebForms postbacks, not React) — every search/filter/page-change action reloads the page. Call **wait_for_stable** after submitting a search or clicking a pagination link, BEFORE reading the tree/extracting data, or you'll grab a half-rendered page.
- Prefer **type_text({selector, text, clear:true})** for search/keyword boxes when you can see a stable id/name in the tree (many WebForms inputs have long generated ids like \`ctl00$ContentPlaceHolder1$txtKeyword\` — read it from get_accessibility_tree/get_interactive_elements first, don't guess it). Otherwise use get_accessibility_tree({filter:"interactive"}) + click_ax/set_field by ref_id. Never use (x,y) coordinates on a search/filter form — these pages are dense and pixel guesses reliably land on the wrong field.
- **Pagination loops**: if "Next"/"›" is clicked and the extracted rows are IDENTICAL to the previous page (same ids/titles), that is a stuck or exhausted paginator, not a slow page — stop, re-orient with get_state_digest, and report what happened rather than clicking Next again.
- **Long/multi-page result sets**: use the progress ledger (progress_update / progress_read) to accumulate found opportunities across pages exactly like the Gmail bulk-task pattern — record each listing's title + posted/due date + a stable id (solicitation/bid number if shown, otherwise the row's detail-page URL). Never hold more than the current page's rows in working memory; let the ledger be the source of truth for what's been collected so far.
- **CAPTCHAs / "verify you are human" / session-expired search forms** are common on government portals. If one appears and CapSolver isn't enabled, stop and surface it to the user rather than retrying the search — retrying almost never clears a CAPTCHA and wastes the whole step budget.
- Many portals reset or expire search-state after a period of inactivity (revisiting an old detail page and going "back" can land on a blank/expired search). If results unexpectedly disappear mid-task, re-run the original search rather than assuming the ledger is wrong.
- These sites rarely have a Gmail-style "select all matching" bulk primitive — per-row extraction via extract_data across pages IS the efficient path here, not a search for a nonexistent bulk-select control.`,
  },
];
