...
- **LARGE TASKS**: If the user goal is big (many items, "all", "sort the inbox", bulk processing), automatically enter chunked mode: keep the OVERALL GOAL + only the NEXT TINY CHUNK (5-8 items). After every small chunk use progress_update (unique IDs) + progress_read / digest. Never try to hold the full list in context.
- Use unique IDs in the ledger: prefer the email subject + date snippet, or the message thread URL/ID from extract_data (e.g. "#inbox/ABC123"). Never use a generic "BidNet Direct email" as the id — every row would collide.
- After any context trim/compaction, always call progress_read before re-scanning so you don't re-process already handled items.
- If the inbox is large, first use Gmail's own search (type into the search box with set_field + submit, e.g. "from:bidnetdirect.com" or "BidNet") to filter before processing.
- Gmail search bar: the input has placeholder="Search mail" (or aria-label). text click({text:"Search mail"}) now auto-prefers the INPUT (avoids ambiguity+coord fallback). Prefer: get_accessibility_tree({filter:"interactive", maxDepth:5}) near top + click_ax({ref_id}) or set_field({ref_id, text: "BidNet Direct", submit: true}) directly. Avoid coord clicks on the toolbar (can trigger heavy virtual list activity).
- Only take action (click, move, label) after you have extracted and ledger-tracked the current visible set.
- **Moving / labeling selected emails**: After using checkboxes or "Select all" to select (you will see blue highlight and count in toolbar), the top action bar changes (shows "Move to", "Labels", trash icons). Click the "Move to" button (or "Labels") **using its ref_id from the current tree**. The menu opens as a popup/portal (often new items appear at the end of the accessibility tree or as a new listbox). **Immediately re-call get_accessibility_tree({filter:"visible"}) or get_state_digest + extract_data to see the new menu items and any filter input inside it.** Then use the fresh ref or type the folder/label name in the popup's input and press Enter / click the target. Do NOT guess coords from screenshot for the toolbar or menu — the refs change when selection state changes. If the menu doesn't appear after click, re-read the tree first before retrying.`,
  },
  {
    name: 'google-docs',
    category: 'general',
